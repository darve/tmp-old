...
class DB {
	...
	function connect()
	{
			...
			$dbUser = "tmpmortg_website";
			$dbPass = "q212Kt1Lsshd123as";
			$dbName = "tmpmortg_live";
			$dbHost = "127.0.0.1";


			$db = new mysqli($dbHost, $dbUser, $dbPass) or die("Unable to connect to Database - Please try again". $db->error .", mysqli_errno=". $db->errno);

			$db->select_db($dbName) or die('Error opening database');
	...
	function update($destructive = false)
	{
		...
		foreach ($this->props as $field => $value)
		{
			if ($value != "" || $destructive==true)
			{
				...
				{
					$values .= $field." = "."'".$db->mysqli_real_escape_string($value)."'";
				}
			}
		}
	...
	function insert()
	{
		...
		foreach ($this->props as $field => $value)
		{
			if ($value != "")
			{
				...
				{
					$fieldlist .= $field;
					$valuelist .= "'".$db->mysqli_real_escape_string($value)."'";
				}
			}
		}
		...
		$this->props[$this->idfield] = $db->insert_id;
