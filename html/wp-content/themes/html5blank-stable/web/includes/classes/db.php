...
class DB {
	...
	function connect()
	{
			...
			$db = new mysqli($dbHost, $dbUser, $dbPass) or die("Unable to connect to Database - Please try again". $db->error .", mysql_errno=". $db->errno);
	...
	function update($destructive = false)
	{
		...
		foreach ($this->props as $field => $value)
		{
			if ($value != "" || $destructive==true)
			{
				...
				{
					$values .= $field." = "."'".$db->escape_string($value)."'";
				}
			}
		}
	...
	function insert()
	{
		...
		foreach ($this->props as $field => $value)
		{
			if ($value != "")
			{
				...
				{
					$fieldlist .= $field;
					$valuelist .= "'".$db->escape_string($value)."'";
				}
			}
		}
